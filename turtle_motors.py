...
try: 
    while 1:
        ...
        if key_input == ' ':
            ...
            qd = 3.14/180 * np.array([150.0]).reshape(-1,1)
            tau_d = np.array([25.0]).reshape(-1,1)
            f_e = 0
            ...
            while 1:
                if kbhit():
                    ...
                    ser.close()
                    print("[Q KEY PRESSED] : All motors stopped\n")
                    save_data(q_data, q2_data, qd, tau_data, timestamps, dt_loop)
                    break
                else:
                    t = time.time()
                    q = np.array(Joints.get_position()).reshape(-1,1)
                    ...
                    # Fkd = (k*((L1*cos(beta1)*(sin(qd) - sin(beta1)**2*sin(qd)) - L1*cos(beta1)*sin(beta1)**2*sin(qd))*(y01 - y02) + (L1*sin(beta1)*(sin(qd) - cos(beta1)**2*sin(qd)) - L1*cos(beta1)**2*sin(beta1)*sin(qd))*(z01 - z02) + (2*(L1*cos(beta1)*(sin(qd) - sin(beta1)**2*sin(qd)) - L1*cos(beta1)*sin(beta1)**2*sin(qd))*(L1*cos(beta1)*(cos(qd) - sin(beta1)**2*(cos(qd) - 1)) - L2*cos(beta2)*(cos(q2_fb) - sin(beta2)**2*(cos(q2) - 1)) - L1*cos(beta1)*sin(beta1)**2*(cos(qd) - 1) + L2*cos(beta2)*sin(beta2)**2*(cos(q2) - 1)))/3 + (2*(L1*sin(beta1)*(sin(qd) - cos(beta1)**2*sin(qd)) - L1*cos(beta1)**2*sin(beta1)*sin(qd))*(L1*sin(beta1)*(cos(qd) - cos(beta1)**2*(cos(qd) - 1)) - L2*sin(beta2)*(cos(q2) - cos(beta2)**2*(cos(q2) - 1)) - L1*cos(beta1)**2*sin(beta1)*(cos(qd) - 1) + L2*cos(beta2)**2*sin(beta2)*(cos(q2) - 1)))/3 - (2*(L1*cos(beta1)**2*cos(qd) - L1*cos(qd)*sin(beta1)**2)*(L1*sin(qd)*cos(beta1)**2 - L2*sin(q2)*cos(beta2)**2 - L1*sin(qd)*sin(beta1)**2 + L2*sin(q2)*sin(beta2)**2))/3))/2
                    tau = 100*((qd-q)) + 15*(- dq) + Fkd
                    # Kd = 7 for pure feedback
                    # tau = 60*((qd-q)) + 12*(- dq)
                    # 10 and 12.5 work decent
                    tau = 10*(Fk - tau_d) + (Fkd) - 15*dq + 10*f_e
